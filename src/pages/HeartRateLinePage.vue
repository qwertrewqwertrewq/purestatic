...
const heartRateData = [
  62, 68, 60, 72, 58, 70, 63, 75, 61, 74, 66, 78, 64, 77, 69, 81, 72, 84, 92, 88,
  95, 90, 97, 93, 99,
]
...
const points = computed(() => {
  return heartRateData.map((value, index) => {
    const x = padding + (index / (heartRateData.length - 1)) * plotWidth
    const y = padding + (1 - value / 110) * plotHeight
    return { x, y }
  })
})
...
<template>
  <main class="line-page">
    <section class="chart-wrap">
      <h2 class="chart-title">心率波动折线</h2>

      <svg class="chart" :viewBox="`0 0 ${width} ${height}`" role="img" aria-label="心率波动折线图">
        ...
        <polyline :points="visiblePointsText" class="heart-line" />
        <circle :cx="tipPoint.x" :cy="tipPoint.y" r="7" class="tip-dot" />
        <!-- <text :x="warningStartX + 10" :y="padding + 18" class="warning-label">高压区</text> -->
      </svg>

      <p class="chart-note">波动更密集，最后20%区间整体均值明显提高。</p>
    </section>
  </main>
</template>
